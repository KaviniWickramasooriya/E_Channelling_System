...
public class AppointmentDAO {
	...
	public boolean updateAppointment(Appointment ap) {
		boolean f = false;
		try {
			String sql = "update appointment set fullName=?,gender=?,age=?,appoint_date=?,diseases=?,doctor_id=?,status=? where id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			...
			ps.setInt(6, ap.getDoctorId());
			ps.setString(7, ap.getStatus());
	...
	public boolean updateRecord(int id, String disc) {
	    boolean f = false;
	    try {
	        String sql = "update appointment set record_disc=? where id=?";
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ps.setString(1, disc);
	        ps.setInt(2, id);  // Set the "id" parameter
